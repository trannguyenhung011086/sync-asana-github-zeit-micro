...
function match(toMatch = '') {
    ...
    if (result)
        return result.map(
            (item) =>
                item.replace('#', '').replace('ref', '').replace('/', '') || '',
        );
}
...
module.exports = {
    ...
    getAsanaIds: async (data) => {
        ...
        console.log('PR data', checks);

        checks.forEach((item) => {
            match(item).forEach((res) => {
                ids.push(res);
            });
        });
        ...
        const commits = await getCommits(data);
        for (const commit of commits) {
            const matchCommit = match(commit['commit']['message']);
            for (const item of matchCommit) {
                ids.push(item);
            }
        }
        ...
        for (const comment of comments) {
            const matchComment = match(comment['body']);
            for (const item of matchComment) {
                ids.push(item);
            }
        }

        if (ids.length === 0) throw new Error('No Asana task ID found!');
