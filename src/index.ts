...
import AsanaActionFactory from './core/actions/AsanaActionFactory'
...
export const handler = async (event, context) => {
	...
	const actionFactory = new AsanaActionFactory(asanaClient)
	...
	try {
		...
		return {
			statusCode: 500,
			body: JSON.stringify(
				`Error occurred: ${error.name}. Message: ${error.message}. Stack: ${error.stack} File: ${error.fileName}`
			),
			logStreamName: context.logStreamName
		}
	}
